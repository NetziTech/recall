...
import { UnlockEncryptionUseCase } from "../../../../src/modules/encryption/application/use-cases/unlock-encryption.use-case.ts";
...
describe("UnlockEncryptionUseCase", () => {
  it("returns Ok(config) on correct passphrase", async () => {
    const config = rehydratedConfig();
    const repo = new StubRepo(config);
    const useCase = new UnlockEncryptionUseCase(
      repo,
      okKdf,
      successCipher,
      acceptingValidator,
    ...
    const result = await useCase.unlock({
      workspaceId: WorkspaceId.from(WS_ID),
  ...
  it("returns Err(NotInitialized) when no config exists", async () => {
  ...
  it("returns Err(KeyValidationFailed) when AEAD auth fails on every envelope", async () => {
    const config = rehydratedConfig();
    const repo = new StubRepo(config);
    const useCase = new UnlockEncryptionUseCase(
      repo,
  ...
  it("returns Err(KeyValidationFailed) when validator rejects", async () => {
    const config = rehydratedConfig();
    const repo = new StubRepo(config);
    const useCase = new UnlockEncryptionUseCase(
      repo,
  ...
  it("rethrows non-AEAD errors from cipher", async () => {
    const config = rehydratedConfig();
    const repo = new StubRepo(config);
