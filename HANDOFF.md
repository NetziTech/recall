...
| Item | Estado |
|---|---|
| **Fecha del handoff** | 2026-05-03 (post-Phase-15 + follow-up — `release/0.1.2-beta.6` cortado tras squash-merge de PR #37 (carryover cosmetico `serverInfo.version` cerrado: handshake JSON-RPC ahora lee `version` de `package.json` at runtime, no del literal hardcoded que se desactualizo en beta.4 y beta.5). Behavior-preserving — sin cambios en tool paths. Bumps de version + release notes nuevas + READMEs/SECURITY actualizados. PR a main pendiente. Ultimo release planeado antes de promover `0.1.2` stable. Ver §6.20) |
...
| **Fase actual** | **`release/0.1.2-beta.6` cortada (carryover cosmetico cerrado via PR #37).** Fix shipped: nueva funcion `resolvePackageVersion()` en `code/src/bootstrap/composition-root.ts` lee `version` desde `package.json` at boot con resolver path-aware (sigue B-CLI-5 pattern para npm-global symlinks); validation `name === "@netzi/recall"` evita falsos positivos cross-package (caught por unit test que sin el name guard retornaba `1.1.1` de vitest). Refactor round 2 extrajo `collectFsCandidates(builder)` + `readPackageVersionField(candidate)` para matar Sonar S3776 (cognitive complexity) + S4144 (duplicacion con `resolveDefaultMigrationsDir`). 5+1 EXIT=0 + SonarQube quality gate PASSED (coverage overall 96.4%, ratings A/A/A). 2560 tests passing en 212 archivos (+3 vs 2557 baseline beta.5). PR a main pendiente, luego tag + GitHub pre-release + `npm publish --tag beta` por usuario, luego smoke contra dogfood DB. **Ultimo release planeado antes de promover `0.1.2` stable**: pendiente soak 24-48h en uso real antes de cortar `release/0.1.2`. |
| **Lineas de codigo** | ~61,300 en `code/src/` + ~36,400 LOC de tests en **212 archivos test**. 8 modulos + shared + composition + bootstrap. **Phase-15 follow-up deltas (PR #37)**: ~+115 LOC netos en `code/src/bootstrap/composition-root.ts` (helpers `collectFsCandidates` + `resolvePackageVersion` + `readPackageVersionField` + refactor de `resolveDefaultMigrationsDir`), +67 LOC unit test nuevo, +29 LOC tightening en E2E + harness. |
...
| **Lineas de documentacion** | ~8,650 en `docs/` (incluye ADR-001..004, convencion `.port.ts` §3.1). **7 release notes** (`RELEASE-NOTES-v0.1.0.md`, `v0.1.1.md`, `v0.1.2-beta.0.md`, `v0.1.2-beta.3.md`, `v0.1.2-beta.4.md`, `v0.1.2-beta.5.md`, **`v0.1.2-beta.6.md`** — nueva). docs/02 §4.3 documenta `min_score`. |
...
| **Tests** | **2560 passing** en 212 archivos test (+3 vs 2557 baseline beta.5; +1 archivo nuevo `code/tests/unit/bootstrap/composition-root.test.ts` con 3 unit tests del helper `resolvePackageVersion`). Coverage SonarQube **overall 96.4%**, ratings A/A/A. La regla "VALORES no SHAPE" reforzada otra vez en PR #37: tighteneo `expect(typeof === "string")` → `expect().toBe(readPackageJsonVersion())` en E2E; sin esto, el carryover beta.4/beta.5 habria seguido escapando indefinidamente. |
...
| **Paquete npm** | **Canal beta**: `@netzi/recall@0.1.2-beta.5` PUBLICADO (2026-05-02 noche, sigue siendo el latest del canal hasta que beta.6 se publique). `release/0.1.2-beta.6` cortada local; PR a main + tag + GitHub pre-release + `npm publish --tag beta` pendientes. **Canal latest**: `@netzi/recall@0.1.1` sigue deprecada. `v0.1.2-beta.5` se reemplaza por beta.6 en mismo canal. `v0.1.0`/`v0.1.1` deprecadas historicamente. `publishConfig.access=public`. Bins `recall` y `recall-server`. **Caveat cosmetico CERRADO en beta.6**: `serverInfo.version` ya lee de `package.json` at runtime, no de literal hardcoded. Tras smoke post-publish de beta.6, `serverInfo.version === "0.1.2-beta.6"` debe quedar pinned por el E2E test tightened. |
| **Licencia** | MIT (`code/LICENSE`). |
| **Estado del release** | **`release/0.1.2-beta.6` cortada local con todos los docs sincronizados.** PR a main pendiente; CI debe pasar verde antes del merge. Una vez mergeado: tag `v0.1.2-beta.6` → commit del merge + GitHub pre-release con `--notes-file docs/RELEASE-NOTES-v0.1.2-beta.6.md` + `npm publish --tag beta --auth-type=web` (usuario, WebAuthn passkey). Smoke post-publish: validar que `mem.health` y otros tools siguen funcionando + que el handshake `initialize.serverInfo.version` ahora reporta `"0.1.2-beta.6"` (no el `"0.1.2-beta.3"` que reportaba beta.4 + beta.5 por el caveat). Si pasa → soak 24-48h en uso real, despues cortar `release/0.1.2` stable. |
| **Issues GitHub abiertos** | **0** — Carryover cosmetico `serverInfo.version` cerrado en Phase-15 follow-up via PR #37. [#31 B-MCP-8](https://github.com/NetziTech/recall/issues/31) cerrado en Phase-15 via PR #33. [#24 B-MCP-7](https://github.com/NetziTech/recall/issues/24) cerrado en Phase-13 via PR #27. Los 4 originales (B-MCP-2/3/4/5) cerrados en Phase-11 via PRs #17/#18/#19/#20. |
...
| **Proximo paso** | **Publicar beta.6 + soak 24-48h + cortar `release/0.1.2` stable**: PR `release/0.1.2-beta.6` → main (pendiente), CI verde + squash-merge, tag `v0.1.2-beta.6`, GitHub pre-release, `npm publish --tag beta`, smoke post-publish validando `serverInfo.version === "0.1.2-beta.6"`, merge-back develop ← main. Despues, soak en uso real (Claude Code) por 24-48h. Si no surfacea bugs nuevos → cortar `release/0.1.2` (sin sufijo) desde develop, mismo flow pero GitHub release SIN `--prerelease` y `npm publish` SIN `--tag beta` (publica a `latest`), luego `npm deprecate 0.1.0` + `npm deprecate 0.1.1` apuntando a `0.1.2`. **Despues de stable**: roadmap v0.5+ (multi-key envelope, encrypted cold start <500ms, perf >10K, hardening defensivo, swap embedder para tar/fastembed highs, wire-schema cleanup `memoria_db` → `recall_db`). |
...
3. **Roadmap v0.5+** (HANDOFF §7 final): multi-key envelope flow (3 stubs `Pending*`), encrypted cold start <500ms via OS keychain, perf hardening >10K entries (W-3.4-PERF-H1/H2/H3 + W-3.3-PERF-M1/M2), hardening defensivo (atomic gitignore, chmod DB, redact path en err.message, StdioJsonRpcServer buffer cap), swap embedder para cerrar tar/fastembed highs si fastembed no actualiza a tar@7 antes de v0.5, wire-schema cleanup `memoria_db` → `recall_db`.
...
_Ultima actualizacion: 2026-05-03 (Phase-15 follow-up — `release/0.1.2-beta.6` cortada tras PR #37 (carryover cosmetico `serverInfo.version` cerrado: handshake JSON-RPC ahora lee `version` de `package.json` at runtime via nuevo helper `resolvePackageVersion()` con anchor walk pattern de B-CLI-5 + name guard `@netzi/recall`). Refactor round 2 extrajo `collectFsCandidates(builder)` + `readPackageVersionField(candidate)` para matar Sonar S3776/S4144. **Ultimo release planeado antes de promover `0.1.2` stable**: bumps + release notes + READMEs + SECURITY + esta seccion. PR a main + tag + GitHub pre-release + npm publish + smoke contra dogfood DB pendientes (acciones manuales del usuario por branch protection + WebAuthn passkey). Despues, soak 24-48h en uso real. Si pasa → cortar `release/0.1.2` stable.)_
