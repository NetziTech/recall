...
| Item | Estado |
|---|---|
| **Fecha del handoff** | 2026-05-02 noche (post-Phase-15 CERRADO end-to-end — `@netzi/recall@0.1.2-beta.5` PUBLICADO en npm beta channel + smoke validado contra dogfood DB confirma B-MCP-8 fix funcional (queries que retornaban `hits=0` en beta.4 ahora retornan `hits=2` y `hits=1` con beta.5). Merge-back develop ← main via PR #35 mergeado. **0 issues abiertos**. Ver §6.20) |
...
| **Fase actual** | **Phase-15 CERRADA end-to-end. `@netzi/recall@0.1.2-beta.5` PUBLICADO en npm beta channel + smoke validado contra DB del dogfood.** Fix shipped via PR #33 (squash-merge a develop) + release via PR #34 (squash-merge a main, tag `v0.1.2-beta.5` → `4a281f0`, GitHub pre-release publicado, `npm publish --tag beta` por usuario via WebAuthn passkey). Smoke con `mem.recall` contra dogfood DB (64 entries) confirma B-MCP-8 fix end-to-end: query `"GitFlow"` (top_k=5) retorna `total_candidates=2, hits=2` (era `hits=0` en beta.4); query `"embedding worker async"` (top_k=5) retorna `total_candidates=1, hits=1` (era `hits=0`). Merge-back develop ← main via PR #35 cerrado limpiamente. Proximo: si beta.5 no surfacea bugs nuevos en uso real → cortar `release/0.1.2` stable + `npm dist-tag add @netzi/recall@0.1.2 latest` + hard-deprecate `0.1.0`/`0.1.1`. |
...
| **Paquete npm** | **Canal beta**: `@netzi/recall@0.1.2-beta.5` **PUBLICADO** (2026-05-02 noche). `npm view @netzi/recall dist-tags` retorna `{ latest: '0.1.1', beta: '0.1.2-beta.5' }`. Tarball: ~6.6 MB, 16 archivos. **Canal latest**: `@netzi/recall@0.1.1` sigue deprecada (latest no se promueve hasta cortar `0.1.2` stable). `v0.1.2-beta.4` superseded por beta.5 en mismo canal. `v0.1.0`/`v0.1.1` deprecadas historicamente. `publishConfig.access=public`. Bins `recall` y `recall-server`. **Caveat cosmetico carryover** (confirmado en smoke beta.5): `serverInfo.version` en el handshake JSON-RPC sigue reportando `"0.1.2-beta.3"` aunque el binario instalado sea beta.5. Busqueda inicial pendiente — probablemente hardcoded en algun adapter del bootstrap o packaging tsup. Plan: `grep -rn "0.1.2-beta" code/src` antes de cortar 0.1.2 stable. |
| **Licencia** | MIT (`code/LICENSE`). |
| **Estado del release** | **PUBLICADO + smoke validado completo del B-MCP-8 fix.** `@netzi/recall@0.1.2-beta.5` en npm beta channel. Tag `v0.1.2-beta.5` → commit `4a281f0` (= main HEAD post squash-merge PR #34). GitHub pre-release publicado: https://github.com/NetziTech/recall/releases/tag/v0.1.2-beta.5. Smoke post-publish con DB real del dogfood (64 entries pre-existentes, 64 vectores poblados): query `"GitFlow"` (top_k=5) → `total_candidates=2, hits=2 ✅` (top decision sobre GitFlow score=0.666); query `"embedding worker async"` (top_k=5) → `total_candidates=1, hits=1 ✅` (top learning B-MCP-3 score=1.287, boost por severity critical). Las queries puramente semánticas (sin overlap lexical) siguen retornando `total_candidates=0` (BM25 no matchea, vector search threshold no las trae) — esto NO es B-MCP-8, es comportamiento esperado del hybrid search. **Merge-back develop ← main via PR #35 cerrado limpiamente** (76a9503). |
...
| **Proximo paso** | **Cortar `release/0.1.2` stable**: si beta.5 no surfacea bugs nuevos en uso real (Claude Code mediante el cliente MCP `recall`), cortar release branch sin sufijo desde develop, mismo flow que beta.5 (bumps + release notes + READMEs/SECURITY/HANDOFF), PR a main, tag `v0.1.2`, GitHub release stable (NO `--prerelease`), `npm publish` (sin `--tag beta` → publica a `latest`), `npm deprecate @netzi/recall@0.1.0 "..."` + `npm deprecate @netzi/recall@0.1.1 "..."` con mensaje apuntando a `0.1.2`. **Antes de cortar stable**, resolver el caveat cosmetico del `serverInfo.version`: `grep -rn "0.1.2-beta" code/src` para encontrar el hardcoded; probable culpable: alguna constante en el handler de `initialize` o en el packaging tsup. PR docs-only para el fix del caveat. **Despues de stable**: empezar roadmap v0.5+ (multi-key envelope, encrypted cold start <500ms, perf >10K, hardening defensivo, swap embedder, wire-schema cleanup `memoria_db` → `recall_db`). |
...
_Ultima actualizacion: 2026-05-02 noche (cierre Phase-15 END-TO-END — `@netzi/recall@0.1.2-beta.5` PUBLICADO en npm beta channel via PRs #33 → #34 → #35. Tag `v0.1.2-beta.5` → `4a281f0` (= main HEAD). GitHub pre-release publicado. Smoke post-publish contra dogfood DB confirmo B-MCP-8 fix end-to-end: query `"GitFlow"` retorna `hits=2` (era 0 en beta.4); query `"embedding worker async"` retorna `hits=1` (era 0). Merge-back develop ← main via PR #35 mergeado. **0 issues abiertos**. Proximo: investigar caveat cosmetico `serverInfo.version` (sigue reportando `0.1.2-beta.3` aunque binario sea beta.5) + cortar `release/0.1.2` stable cuando beta.5 valide en uso real 24-48h.)_
