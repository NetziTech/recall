...
| Item | Estado |
|---|---|
| **Fecha del handoff** | 2026-05-03 (Phase-16 — **`release/0.1.2` STABLE cortada** tras smoke fresh 10/10 PASS de beta.6. Channel promotion sin cambios de codigo: misma base que `0.1.2-beta.6` (commit `f3aca46` re-tagged como `v0.1.2`). Bumps + release notes consolidando todo el cycle beta.0..beta.6 + READMEs/SECURITY/HANDOFF reflejando "stable" + nueva §6.21. PR a main pendiente. Steps post-merge: tag `v0.1.2` (sin sufijo), GitHub release **stable** (NO `--prerelease`), `npm publish` **sin `--tag beta`** (publica a `latest`), `npm deprecate 0.1.0` + `npm deprecate 0.1.1`. Ver §6.21) |
...
| **Fase actual** | **`release/0.1.2` STABLE cortada (channel promotion sin cambios de codigo desde beta.6).** Decision del usuario: saltar el soak 24-48h y promover directo, justificado por (1) fresh smoke 10/10 PASS contra workspace nuevo (incluyendo `serverInfo.version === "0.1.2-beta.6"`), (2) cero issues abiertos al cierre de Phase-15 follow-up, (3) los tests 2560/2560 + Sonar quality gate PASSED en cada uno de los 7 PRs del cycle. Bumps: `code/package.json` 0.1.2-beta.6 → 0.1.2; `code/sonar-project.properties` igual. `docs/RELEASE-NOTES-v0.1.2.md` (NEW) consolida el cycle completo + tabla de migracion para usuarios viniendo de `0.1.0`/`0.1.1`/`0.1.2-beta.*`. READMEs reescritos: banner "stable" en lugar de "beta", install command sin `@beta`. SECURITY.md tabla con `0.1.2` como unica supported. PR a main pendiente. **Despues del merge: tag `v0.1.2`, GitHub release stable (no `--prerelease`), `npm publish` sin `--tag beta` (publica directo a `latest`), `npm deprecate 0.1.0` + `0.1.1` con mensaje apuntando a `0.1.2`.** |
...
| **Paquete npm** | **Canal latest** (post-Phase-16): `@netzi/recall@0.1.2` STABLE pendiente de publicar (release branch local lista, PR a main pendiente). Tras publish: `npm view @netzi/recall dist-tags` retornara `{ latest: '0.1.2', beta: '0.1.2-beta.6' }`. `0.1.2-beta.6` queda en canal beta (no se elimina ni deprecate, simplemente no es la version recomendada). `0.1.0` + `0.1.1` se hard-deprecate via `npm deprecate ... "..."` apuntando a `0.1.2`. `publishConfig.access=public`. Bins `recall` y `recall-server`. **Carryover cerrado en beta.6 + validado en stable**: `serverInfo.version` lee de `package.json` at runtime — instalar `@netzi/recall@latest` (= 0.1.2) hara que el handshake reporte `"0.1.2"`. |
| **Licencia** | MIT (`code/LICENSE`). |
| **Estado del release** | **`release/0.1.2` STABLE cortada local con todos los docs sincronizados.** PR a main pendiente; CI debe pasar verde antes del merge. Una vez mergeado (steps manuales del usuario): tag annotated `v0.1.2` → commit del merge (con `git switch --detach v0.1.2` por hook), GitHub release **stable** (`gh release create v0.1.2 --target main --notes-file docs/RELEASE-NOTES-v0.1.2.md` — SIN `--prerelease`), `cd code && npm publish --auth-type=web` (SIN `--tag beta` → publica directo a `latest`), `npm deprecate @netzi/recall@0.1.0 "..."` + `npm deprecate @netzi/recall@0.1.1 "..."` con mensaje "use @netzi/recall@latest (now 0.1.2)". Smoke post-publish: `npx @netzi/recall@latest --help` debe ejecutar 0.1.2 (no 0.1.1). Merge-back develop ← main. **Despues**: roadmap v0.5+. |
...
| **Memoria propia** | **POBLADA por dogfood, queue DRENADA, vectores listos, B-MCP-8 fix confirmado end-to-end** — `<repo>/.recall/recall.db` tiene 64 entries (27 decisions + 23 learnings + 11 entities + 0 tasks + 3 turns), `schema_version=8`, modo `private`. **embedding_queue: 0 pendientes**. **embedding_metadata: 64 vectores poblados**. Smoke beta.5 confirmo `mem.recall` con literal+paraphrased matches retorna hits no-vacios (B-MCP-8 cerrado en produccion). El semantic recall en queries puramente abstractas (sin overlap lexical) sigue dependiendo del ranking de cosine + threshold del hybrid scorer — funciona donde aplica. **Hooks PR #26** siguen activos. |
...
| **Proximo paso** | **Publicar `0.1.2` stable**: CI verde en PR `release/0.1.2` → main, squash-merge, tag `v0.1.2`, GitHub release stable (no `--prerelease`), `npm publish` (no `--tag beta`), `npm deprecate 0.1.0` + `0.1.1` apuntando a `0.1.2`, smoke fresh post-publish (`npx @netzi/recall@latest --help` debe ejecutar 0.1.2), merge-back develop ← main. **Despues de stable**: empezar roadmap v0.5+. Items planeados (HANDOFF §6.21): (1) multi-key envelope flow (3 stubs `Pending*`), (2) encrypted cold start `<500ms` via OS keychain, (3) perf hardening >10K entries, (4) hardening defensivo (atomic gitignore, chmod DB, redact path), (5) swap embedder o tar@7 para cerrar 2 highs upstream, (6) wire-schema cleanup `memoria_db` → `recall_db`. Fase 0.5+ no tiene fecha — se prioriza por uso real reportado por usuarios externos. |
...
| 9 | PR `release/0.1.2` → main | orquestador | Push branch + `gh pr create --base main`. Conflicts esperados (mismos archivos del PR #38 + esta vez con `code/src/bootstrap/composition-root.ts` ya en main). Resolucion `--ours` (release branch = canonica post-release). |
| 10 | Tag annotated `v0.1.2` + push (con `git switch --detach v0.1.2` por hook) | usuario | Steps manuales por branch protection. |
| 11 | GitHub release **stable** | usuario | `gh release create v0.1.2 --target main --notes-file docs/RELEASE-NOTES-v0.1.2.md` — **SIN `--prerelease`**. |
| 12 | `npm publish` | usuario | `cd code && npm publish --auth-type=web` — **SIN `--tag beta`** → publica a `latest`. WebAuthn passkey. |
| 13 | `npm deprecate 0.1.0` + `npm deprecate 0.1.1` | usuario | Mensaje: "deprecated due to bugs B-MCP-1..8 (Phase-7..15). Use @netzi/recall@latest (now 0.1.2)." |
| 14 | Smoke fresh post-publish | orquestador o usuario | `npx --yes @netzi/recall@latest --help` debe ejecutar 0.1.2 (no 0.1.1). `npm view @netzi/recall dist-tags` debe retornar `{ latest: '0.1.2', beta: '0.1.2-beta.6' }`. |
| 15 | Merge-back develop ← main via PR `chore/sync-develop-after-0.1.2` | orquestador | Conflicts esperados, resolucion `--theirs` (main = canonica). |
...
4. **El smoke fresh post-publish del stable es identico al del beta.6** (mismo binary). Pero **debe correrse igual** porque valida que el `npm publish` real no introdujo ningun problema de empaquetado (e.g. `bin` paths rotos, files faltantes en `files: [...]` del package.json) que el `npm pack --dry-run` podria no atrapar.

### Estado del repo post-Phase-16 (pre-PR a main)

| Item | Valor |
|---|---|
| **HEAD de `main`** | `f3aca46` (= tag v0.1.2-beta.6; pendiente de avanzar al merge commit del PR `release/0.1.2`) |
| **HEAD de `develop`** | `28cbb94` (post merge-back de beta.6 via PR #39) |
| **HEAD de `release/0.1.2`** | bumps + release notes + READMEs + SECURITY + HANDOFF (este commit) |
| **Tag mas reciente** | `v0.1.2-beta.6` → `f3aca46` (sin cambios; `v0.1.2` pendiente) |
| **GitHub release mas reciente** | https://github.com/NetziTech/recall/releases/tag/v0.1.2-beta.6 (sin cambios; v0.1.2 stable pendiente) |
| **npm dist-tags** | `{ latest: '0.1.1', beta: '0.1.2-beta.6' }` (sin cambios; cambia a `{ latest: '0.1.2', beta: '0.1.2-beta.6' }` post-publish) |
| **Issues abiertos** | **0** |
| **PRs abiertos** | 0 (excepto este de release Phase-16 pendiente de abrir) |
...
_Ultima actualizacion: 2026-05-03 (Phase-16 — **`release/0.1.2` STABLE cortada** desde develop tras decision del usuario de saltar el soak post-beta.6 (justificado por fresh smoke 10/10 PASS + cero issues). Channel promotion sin cambios de codigo: misma base que `0.1.2-beta.6` (commit `f3aca46`). Bumps + release notes consolidando todo el cycle beta.0..beta.6 + READMEs/SECURITY/HANDOFF reflejando "stable" + nueva §6.21. PR a main pendiente. Steps post-merge (manuales del usuario): tag annotated `v0.1.2`, GitHub release **stable** (NO `--prerelease`), `npm publish` **SIN `--tag beta`** (publica directo a `latest`), `npm deprecate 0.1.0` + `npm deprecate 0.1.1`. **Primer release stable de `@netzi/recall`** — abre formalmente el v0.5+ roadmap.)_
